...
def padding_img(img, filter_size=3):
    ...
    return np.pad(np.array(img), filter_size, mode="edge")    
...
def mean_filter(img, filter_size=3):
    ...
    img = padding_img(img, filter_size=filter_size//2)
    ...
    return smoothed_img.astype(np.double)
...
def show_res(origin_img, before_img, after_img, title):
    """
...
if __name__ == '__main__':
    ...
    ori_img = read_img(img_gt)
    filter_size = 3

    # Mean filter
    mean_smoothed_img = mean_filter(img, filter_size)
    psnr_score = psnr(ori_img, mean_smoothed_img)
    title = 'ex1_images/mean_filter_{:.10f}.jpg'.format(psnr_score)
    show_res(ori_img, img, mean_smoothed_img, title)
    print('PSNR score of mean filter: ', psnr_score)

    # Median filter
    median_smoothed_img = median_filter(img, filter_size)
    psnr_score = psnr(ori_img, median_smoothed_img)
    title = 'ex1_images/median_filter_{:.10f}.jpg'.format(psnr_score)
    show_res(ori_img, img, median_smoothed_img, title)
    print('PSNR score of median filter: ', psnr_score)
